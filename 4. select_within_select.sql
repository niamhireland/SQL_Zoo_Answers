...
-- 4. Show the name and population of the country that has a population that is more than United Kingdom but less than Germany.
SELECT name, population 
FROM world 
WHERE population < (SELECT population 
FROM world WHERE name IN ('Germany')) 
AND population > (SELECT population 
FROM world WHERE name IN ('United Kingdom'))

-- 5. Show the name and the population of each country in Europe. Show the population as a percentage of the population of Germany.
SELECT name, CONCAT(CAST(ROUND(population/ (SELECT population 
FROM world WHERE name = 'Germany') * 100, 0) AS INT), '%') AS percentage
